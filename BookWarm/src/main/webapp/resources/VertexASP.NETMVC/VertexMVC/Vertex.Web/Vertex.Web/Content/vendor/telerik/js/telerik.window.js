(function ($) {
    ...
    $t.window = function (element, options) {
        ...
        this.bringToTop();
        $element.bind("mousedown", $.proxy(this.bringToTop, this));
    ...
    $t.window.prototype = {
        ...
        center: function () {                       
            ...
            var wScrLeft = $window.scrollLeft();
            var bScrLeft = $body.scrollLeft();
            var wScrTop = $window.scrollTop();
            var bScrTop = $body.scrollTop();

            if (jQuery.browser.webkit && wScrTop == bScrTop) {
                bScrTop = 0;
                bScrLeft = 0;
            }

            $element.css({
                left: wScrLeft + bScrLeft + Math.max(0, ($window.width() - $element.width()) / 2),
                top: wScrTop + bScrTop + Math.max(0, ($window.height() - $element.height()) / 2)
            });
        ...
        bringToTop: function () {
            ...
            $(".t-window").each(function() {
                ...
                if (this != currentWindow.element && wnd.find(".t-window-content > iframe").length > 0) {
    ...
    $.extend($t.window, {
        create: function () {
            ...
            if (element) {
                ...
                return $(windowHtml.string()).appendTo(document.body).eq(0).tWindow(options);
            }
        },
